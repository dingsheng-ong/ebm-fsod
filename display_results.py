...
def tabulate(data: dict, split: int, shot: int, file=None):
    ...
    for i in range(nrow):
        ...
        write_fn(
            f'│ {data["seed"][i]:02d} │{"│".join([f"{v:^6.1f}" for v in values])}│\n'
        )

    if nrow > 0:
        write_fn(f'├────{"┼──────" * ncol}┤\n')
        mean = [np.nanmean(data[k]) for k in data]
        # serr = [1.96 * float(sem(data[k], nan_policy="omit")) for k in data]
        serr = [float(1.96 * np.std(data[k]) / (len(data[k]) ** 0.5)) for k in data]
    ...
    if is_coco:
        ...
        for metric in ["AP", "bAP", "nAP"]:
            ...
            for shot in [1, 2, 3, 5, 10, 30]:
                if data.get(("coco", None, shot, "defrcn_gfsod_r101_novel")):
                    score = f"{np.mean(data[('coco', None, shot, 'defrcn_gfsod_r101_novel')][f'{metric}']):.1f}"
                    if len(score) < 4:
        ...
        for metric in ["AP50", "bAP50", "nAP50"]:
            ...
            for split in [1, 2, 3]:
                for shot in [1, 2, 3, 5, 10]:
                    if data.get(("voc", split, shot, "defrcn_gfsod_r101_novel")):
                        score = f"{np.mean(data[('voc', split, shot, 'defrcn_gfsod_r101_novel')][f'{metric}']):.1f}"
                        if len(score) < 4:
                            score = (" " * (5 - len(score))) + score
                    else:
                        score = "-"
...
def main(args):
    ...
    results = {}
    for setting in ["defrcn_gfsod_r101_novel"]:
        ...
        for split in splits:
            ...
            for shot in shots[dataset]:
                ...
                shot_dir = os.path.join(setting_dir, f"tfa-like")
                if not os.path.exists(shot_dir):
                    continue
                for seed in args.seed:
                    ckpt_dir = os.path.join(shot_dir, f"{shot}shot_seed{seed}")
    ...
    # t = [v["nAP50"][0] for v in results.values()]
    # print(f"nAP50: {np.mean(t):.2f}+/-{sem(t) * 1.96 / len(t) ** 0.5:.3f}")
...
if __name__ == "__main__":
    ...
    if len(args.seed) == 1:
        seed = args.seed[0]
        if "-" in seed:
            a, b = map(int, seed.split("-"))
            args.seed = list(range(a, b + 1))
        else:
            args.seed = [int(seed)]
    else:
